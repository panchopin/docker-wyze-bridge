...
- Wait for the authenticated bridge camera catalog and semantic `/api/ready` state before declaring startup ready, preventing empty boot-time catalogs from wiping native `go2rtc` aliases.
- Validate snapshot replacements by content hash and image sanity so empty, tiny, non-image, stale, or byte-identical frames do not replace the cached still image.
- Treat `SD_ONLY=true` as a hard Home Assistant mode: one SD feed per camera, HD controls rejected or hidden, and only `*-sd` native aliases exposed in the production proof path.
- Keep recently healthy WHEP streams in a bounded recovering state during short reconnects, then recreate them if media does not return.
- Add release packaging hygiene so local agent notes, scratch files, option payloads, SSH env files, and private SDK values are excluded from public artifacts.
- Record the current proof boundary: Phase 1 snapshots, Phase 2 startup/API, Phase 3 SD-only, and Phase 5 overlay/API are green; the strict Phase 4 hour gate remains blocked by intermittent Frigate/Scrypted skipped-FPS blips even though WHEP media stayed ready.
